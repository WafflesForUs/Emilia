...
function detector(i,message,client)
    local vals = {}
    for i, v in io.popen(
        "ffprobe -v error -show_entries frame=width,height -select_streams v -of csv=p=0 " .. i.url
    ):lines() do --video/gif resolution per frame
    ...
    if (IsSketchy(vals) or 100) < 70 then
        message:delete()
        message:reply("dont post crash gifs/videos "..message.author.mentionString.."\n\n||"..client:getUser("411887008160415766").mentionString.."||")
    end
end
