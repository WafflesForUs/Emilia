    ...
    for _, file in pairs(fs.readdirSync(dir)) do
        ...
        if text and DelUnwantedCharacters(message.content:lower()):match(file:match("(%w+)")) then
            ...
            if punishment == "ban" or punishment == "kick" then
                message.guild[punishment .. "User"](
                    message.guild,
                    message.author.id,
                    "auto moderation: said " .. file,
                    7
                )
                local msg = client:getChannel("855773180312420372"):send(".")
                logs:New(msg, punishment, message.author, 15158332)
                msg:delete()
