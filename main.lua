...
client:on("messageCreate",function(message) 
    if message.guild and not message.author.bot and not message.guild:getMember(message.author.id):hasRole("773838374440009758") then
        iw(message) 
    end 
end)-- ^
...
client:on(
    ...
    function(message)
        ...
        if string.sub(arg, 1, #(prefix)) == prefix and message.guild and not message.author.bot then
