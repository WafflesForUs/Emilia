...
return function(...)
    ...
    if message.mentionedUsers.first or user then
        local kiss = json.parse(spawn("curl", {args = {"http://api.nekos.fun:8080/api/lick"}, stdio = {nil, true, 1}}).stdout.read())
        local responses={" is licking "," is getting too close to "," licks "}
        message:reply {
            embed = {
