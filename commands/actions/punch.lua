...
return function(...)
    ...
    if message.mentionedUsers.first or user then
        local req = json.parse(spawn("curl", {args = {"https://shiro.gg/api/images/punch"}, stdio = {nil, true, 1}}).stdout.read())
        local responses={" is pucnhing "," is hurting "," landed a hit on "}
        message:reply {
            embed = {
