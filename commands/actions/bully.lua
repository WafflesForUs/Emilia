...
return function(...)
    ...
    if message.mentionedUsers.first or user then
        local req = json.parse(spawn("curl", {args = {"https://api.waifu.pics/sfw/bully"}, stdio = {nil, true, 1}}).stdout.read())
        local responses={" is bullying "," is being rude to "," is such a meanie to "}
        message:reply {
            embed = {
